...
class ProductController extends Controller
{
    ...
    public function show(int $id): JsonResponse
    {
        $product = Product::with([
            'category.parent.parent',
            'categories:id,name,slug,parent_id',
            // Two levels, so a chip can name a shelf the way every other
            // screen does — several are called Accessories, and one ancestor
            // is not always enough to tell them apart.
            'categories.parent:id,name,parent_id',
            'categories.parent.parent:id,name',
    ...
    /**
     * Start a new product from one that already exists.
     *
     * Entering one from scratch is six panels of work, and most of a shop's
     * range is near-identical to something already on it: the same shelf, the
     * same warranty wording, the same filter answers, a different processor.
     * Retyping all of that to change one line is the slowest thing about the
     * form, and the most error-prone — the filter answers in particular are
     * easy to skip, and a product that answers none of them is invisible to
     * the sidebar.
     *
     * What is deliberately not copied is anything that identifies the goods
     * rather than describes them:
     *
     *   slug       minted from the new name, and never reused
     *   barcode    the number on this shop's box, unique by definition
     *   sku        the same, per option
     *   stock      a copy has none; stock arrives under Purchasing
     *   is_active  a draft, because a copy is never finished on arrival
     *
     * The name carries a suffix for the same reason: two rows reading exactly
     * alike in the list is the state somebody edits the wrong one from.
     */
    public function duplicate(int $id): JsonResponse
    {
        $source = Product::with([
            'images', 'specifications', 'attributeValues:id',
            'categories:id', 'relatedProducts:id', 'quantityDiscounts',
            'variants',
        ])->findOrFail($id);
        ...
        $copy = DB::transaction(function () use ($source) {
            ...
            $copy = Product::create([
                ...collect($source->getAttributes())
                    ->except([
                        'id', 'slug', 'barcode', 'stock_quantity', 'is_active',
                        'has_variants', 'variant_attributes',
                        'created_at', 'updated_at',
                    ])
                    ->all(),
                'name' => $name,
                'slug' => SlugFactory::unique(Product::class, $name),
                // barcode and slug are excluded above rather than nulled
                // here: one place deciding what a copy may not inherit.
                'stock_quantity' => 0,
                'is_active' => false,
            ]);

            $this->gallery->syncProduct($copy, $source->images->map(fn ($image) => [
                'image_path' => $image->image_path,
                'alt_text' => $image->alt_text,
                'is_primary' => (bool) $image->is_primary,
            ])->all());

            $this->syncSpecifications($copy, $source->specifications->map(fn ($spec) => [
                'group' => $spec->group,
                'name' => $spec->name,
                'value' => $spec->value,
            ])->all());

            // The answers the sidebar filters on, which is the part worth
            // copying most: skipping them leaves a product nobody can narrow to.
            $this->syncAttributeValues($copy, $source->attributeValues->pluck('id')->all());

            $copy->syncCategories($source->categories->pluck('id')->all());
            $this->syncRelated($copy, $source->relatedProducts->pluck('id')->all());

            $this->syncQuantityDiscounts($copy, $source->quantityDiscounts->map(fn ($tier) => [
                'min_quantity' => $tier->min_quantity,
                'price' => $tier->price,
            ])->all());

            if ($source->has_variants) {
                $this->copyOptions($source, $copy);
            }
        ...
        return $this->successResponse(
            $copy->fresh(['variants', 'images', 'categories']),
            "Copied to '{$copy->name}', saved as a draft.",
            201
        );
    }

    /**
     * The options, with everything that identifies a box stripped off.
     *
     * Structure only: a copy has no stock, so the opening figures are zero and
     * the ledger is untouched. SKU and barcode are unique per option and are
     * left for whoever finishes the copy to fill in.
     */
    private function copyOptions(Product $source, Product $copy): void
    {
        app(ProductVariantService::class)->convertToVariants(
            $copy,
            $source->variant_attributes ?? [],
            $source->variants->map(fn ($variant) => [
                'name' => $variant->name,
    ...
    /**
     * "RTX 4090" becomes "RTX 4090 (Copy)", and a copy of that "(Copy 2)".
     *
    ...
    {
        $base = preg_replace('/\s*\(Copy(?:\s+\d+)?\)$/u', '', $name);
        $candidate = "{$base} (Copy)";

        for ($n = 2; Product::where('name', $candidate)->exists(); $n++) {
